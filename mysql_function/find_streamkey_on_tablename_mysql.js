...
module.exports = async function getJSON(tablename, supervNum, name=null, mac=null, num=null) {
  ...
  try {
    ...
    if (supervNum !== null) {
      ...
      rows = (await connection.execute("SELECT "+column0+" , "+column1+" , "+column2+" FROM "+ tablename +" WHERE supervNum= '"+supervNum+"'"))[0];
      ...
      rows = (await connection.execute("SELECT "+column1+" FROM "+ tablename +" WHERE name= '"+name+"' and mac= '"+mac+"' and id= '"+num+"'"))[0];
